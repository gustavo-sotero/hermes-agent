...
""" + _UPSERT_KEEP_EXISTING_SQL,
    ...
    def clear_session_cwd(self, session_id: str) -> Optional[int]:
        ...
        Inverse of :meth:`update_session_cwd`: used when a session is moved to "Home" (no project owns
        it). Clears ``cwd`` plus the git identity columns and bumps ``git_metadata_generation`` in the
        same write, so an async git probe still in flight cannot re-stamp a stale claim for the old
        workspace (same generation contract as the move path). Returns the new generation, or None
        when the row does not exist."""
        if not session_id:
            return None
        if int(self._write_rowcount(
            """UPDATE sessions
               SET cwd = NULL, git_branch = NULL, git_repo_root = NULL,
        ...
        if not rows:
            return None
        try:
            return int(rows[0]["git_metadata_generation"])
    ...
    def list_unbound_sessions(self) -> List[Dict[str, Any]]:
        ...
        These are the rows the sidebar buckets under "Home" (``__no_project__``): messaging rows created
        before the gateway resolved a cwd, and sessions that genuinely never had a workspace. The CLI
        ``sessions backfill`` command feeds on this list."""
        rows = self._read_all(
    ...
    def backfill_session_cwd(self, cwd: str, session_ids: List[str]) -> int:
        ...
        NULL-only write (COALESCE semantics): an explicit cwd already set on a row is never overwritten.
        ...
        return int(self._write_rowcount(
            f"UPDATE sessions SET cwd = ? WHERE id IN ({placeholders}) AND cwd IS NULL",
            (cwd, *ids),
        ) or 0)
