...
class SessionRecoveryMixin:
    ...
    def _record_gateway_session_peer(
        ...
        # Messaging sessions persist a cwd so their rows land under a project in the sidebar instead of
        # "Home" (NULL-only on the DB side — an explicit cwd is never clobbered). Best-effort: resolver
        # failures leave the previous NULL-cwd behavior.
        _peer_cwd = None
        resolver = getattr(self, "cwd_resolver", None)
        if callable(resolver):
            try:
                _peer_cwd = resolver(session_key, source)
            except Exception as exc:
                logger.debug("Session cwd resolver failed for %s: %s", session_key, exc)
            if _peer_cwd and (
                not isinstance(_peer_cwd, str)
                or not os.path.isdir(os.path.abspath(os.path.expanduser(_peer_cwd)))
            ):
                _peer_cwd = None
        try:
            recorder(
                session_id, **peer, display_name=display_name or source.chat_name,
                origin_json=_origin_json(source), cwd=_peer_cwd,
    ...
    def _create_session_row_cwd(self, session_key, origin) -> Optional[str]:
        """The session's working directory for its DB row, or None.

        Delegates to the injected ``cwd_resolver`` (composed by the gateway host: per-session record →
        active project → resolved terminal cwd). Returns None when no resolver is wired or the resolved
        path does not exist locally — callers keep the previous behavior (row stays NULL, session buckets
        under "Home") instead of stamping a junk path."""
        resolver = getattr(self, "cwd_resolver", None)
        if not callable(resolver):
            return None
        try:
            resolved = resolver(session_key, origin)
        except Exception as exc:
            logger.debug("Session cwd resolver failed for %s: %s", session_key, exc)
            return None
    ...
    def _create_session_row(self, session_key, db_create_kwargs, origin, display_name, *, log) -> None:
        ...
        try:
            # Stamp the session's working directory at row creation (NULL-only upstream of here —
            # create_session's INSERT never overwrites an existing value). Failure is non-fatal.
            _row_cwd = self._create_session_row_cwd(session_key, origin)
            if _row_cwd:
                db_create_kwargs = {**db_create_kwargs, "cwd": _row_cwd}
