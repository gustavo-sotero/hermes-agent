...
class GatewayInboundMixin:
    ...
    async def _hm_cmd_init(self, event, source, _quick_key):
        ...
        # The target directory is the SESSION's working directory, not the gateway process cwd (which
        # is HERMES_HOME on desktop installs and would make /init generate AGENTS.md into the
        # install/state tree). Resolve through the same per-session resolver used for terminal tools
        # and sidebar attribution; fall back to the process cwd only when no resolver is wired.
        _init_cwd = None
        try:
