...
class SessionGatewayMixin:
    ...
    def record_gateway_session_peer(
        ...
        origin_json: str = None, cwd: str = None,
        ...
        row. ``cwd`` — when supplied and the row still has no cwd — stamps the session's working directory
        (COALESCE, so an explicit cwd already set on the row is never overwritten). Messaging rows created
        before the gateway resolved a cwd are healed here, which moves them out of the sidebar's "Home"
        bucket and under their project. Self-healing: a missing target row (deferred ``create_session`` write, or crash between routing
            ...
            if conn.execute("SELECT 1 FROM sessions WHERE id = ? LIMIT 1", (session_id,)).fetchone() is None:
                ...
                conn.execute(
                    f"""INSERT INTO sessions (
                               ...
                               profile_name{cwd_cols}, started_at
                           )
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?{cwd_vals}, ?)
                    ...
                    (session_id, source, user_id, session_key, chat_id, chat_type, thread_id, display_name,
                     origin_json, self._own_profile_name(),
                     *((cwd_value,) if cwd_value else ()), time.time()),
                )
